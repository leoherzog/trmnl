...
When content exceeds the height threshold, the limiter adds the `content--small` class and automatically truncates the first overflowing block using the Clamp Engine [Clamp](/framework/docs/clamp) so it fits the remaining space. Subsequent blocks are hidden.
...
[Fit Value Automatically resize numbers and values to fit within their containers](/framework/docs/fit_value)
