...
Tables support five sizes: Base, Large, Small, and XSmall. Use modifier classes to change row heights.
...
Demonstrates the Overflow behavior[Table Overflow](/framework/docs/table_overflow) and trailing “and X more” row when content exceeds the height budget.
...
Apply `data-clamp`[Clamp](/framework/docs/clamp) to each cell’s content to ensure consistent single-line truncation with ellipsis. This works with the Table Overflow behavior [Table Overflow](/framework/docs/table_overflow) .
...
[Chart Visualize data optimized for 1-bit rendering](/framework/docs/chart)
