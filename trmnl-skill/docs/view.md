...
The Layout element[Layout](/framework/docs/layout) is the core component of every View [View](/framework/docs/view) , providing a consistent container for your content.
 Views can optionally include a Title Bar [Title Bar](/framework/docs/title_bar) for additional context.
...
There are four view types: `view--full`, `view--half_horizontal`, `view--half_vertical`, and `view--quadrant`.
 ...
 Other view types must be nested inside a [Mashup](/framework/docs/mashup) component.
...
When multiple plugins share a single screen, each one gets its own view, and those views must be wrapped in a[Mashup](/framework/docs/mashup) container.
