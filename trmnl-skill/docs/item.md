...
The `.list` class is deprecated. Prefer a column component, flex column, grid column, or a layout wrapper with a [Gap](/framework/docs/gap) utility instead. The [Overflow](/framework/docs/overflow) engine still supports legacy `.list` for backward compatibility.

Previous

[Rich Text Display formatted paragraphs with alignment and size variants](/framework/docs/rich_text)
